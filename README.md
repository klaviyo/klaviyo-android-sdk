...
2. Add the dependencies to your app's build file
   <details>
      ...
      // build.gradle.kts
      dependencies {
          implementation("com.github.klaviyo.klaviyo-android-sdk:analytics:4.5.2")
          implementation("com.github.klaviyo.klaviyo-android-sdk:push-fcm:4.5.2")
          implementation("com.github.klaviyo.klaviyo-android-sdk:forms:4.5.2")
          implementation("com.github.klaviyo.klaviyo-android-sdk:location:4.5.2")
      }
      ```
   </details>
...
> **Push tokens create profiles:** Registering a push token creates a profile, and that can happen
> before any call to `setProfile`, `setEmail`, or `setPhoneNumber` — in which case Klaviyo creates an
> anonymous profile and merges it once you provide identifying information. This is expected behavior.
> By default the SDK registers the token whenever FCM issues a new one. To take full control of when
> tokens are registered, set `automatic_push_token_forwarding="false"` and call
> `Klaviyo.setPushToken(...)` yourself — see
> [Option B — Manual integration](#option-b--manual-integration) below.
...
- **Push open tracking** (`automatic_push_open_tracking`) — **opt-in** (off by default). Set it to
  ...
  [Deep Linking](#deep-linking).
- **Push token registration** (`automatic_push_token_forwarding`) — set it to `true` and the SDK
  fetches the current FCM token and registers it with Klaviyo, so you do **not** need to call
  `Klaviyo.setPushToken(...)` yourself.

Set both to `true` for the zero-boilerplate setup.
...
`onNewToken` only when it generates or rotates a token.
...
If you run multiple push providers and don't register `KlaviyoPushService`, set
`automatic_push_token_forwarding="true"` — the fetch at initialize and on each foreground works
without the service, so the SDK still gets the token. Klaviyo message **display** does require the
service.

#### The two flags are independent

`automatic_push_open_tracking` and `automatic_push_token_forwarding` are independent, and either can
be set without the other. Both are opt-in — leave them unset and nothing changes.
...
**both** the fetch at initialize/foreground **and** `KlaviyoPushService.onNewToken` forwarding — no
...
If you set `automatic_push_token_forwarding="true"` (see Option A), the SDK handles this for you and you can skip
this section. The steps below apply when you set it to `"false"`, or when your app supplies its own
`FirebaseMessagingService`.

Forward tokens yourself: call `Klaviyo.setPushToken()` from your own `onNewToken()` (see
