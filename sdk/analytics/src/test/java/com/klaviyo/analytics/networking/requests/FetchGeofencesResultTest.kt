...
internal class FetchGeofencesResultTest : BaseTest() {
    ...
    fun `Success result contains geofence data`() {
        val geofences = listOf(
            FetchedGeofence("id1", 40.7, -74.0, 100.0),
            FetchedGeofence("id2", 50.1, -120.2, 200.0)
        )
    ...
    fun `Success result data is immutable list`() {
        val geofences = listOf(FetchedGeofence("id1", 40.7, -74.0, 100.0))
        val result = FetchGeofencesResult.Success(geofences)
    ...
    fun `Success result data access`() {
        val geofence1 = FetchedGeofence("id1", 40.7, -74.0, 100.0)
        val geofence2 = FetchedGeofence("id2", 50.1, -120.2, 200.0)
    ...
    fun `Success result maintains data ordering`() {
        val geofences = (1..10).map { i ->
            FetchedGeofence("id$i", (40.0 + i), (-74.0 + i), (100.0 * i))
        }
        val result = FetchGeofencesResult.Success(geofences)
    ...
    fun `Success equality based on data`() {
        val geofences1 = listOf(FetchedGeofence("id1", 40.7, -74.0, 100.0))
        val geofences2 = listOf(FetchedGeofence("id1", 40.7, -74.0, 100.0))
        val geofences3 = listOf(FetchedGeofence("id2", 40.7, -74.0, 100.0))
