...
internal class KlaviyoLocationManager : LocationManager {
    ...
    private val observers = Collections.synchronizedList(
        CopyOnWriteArrayList<GeofenceObserver>()
    )
    ...
    private fun notifyObservers(geofences: List<KlaviyoGeofence>) {
        synchronized(observers) {
            observers.forEach { it(geofences) }
        }
    }
     ...
     */
    fun fetchGeofences() {
        Registry.log.info("Fetching geofences for company ${Registry.config.apiKey}")
        ...
        Registry.get<ApiClient>().fetchGeofences { result ->
            when (result) {
                is FetchGeofencesResult.Success -> {
                    ...
                    Registry.log.info("Successfully fetched ${geofences.size} geofences")
